...
function chooseCard(state: GameState): Card {
  ...
  if (state.trick.length === 0) {
    const trumpCards = cards.filter((card) => {
      return effectiveSuit(card, state.trump!) === state.trump
    })
    ...
    if (makerIsLeading && trumpCards.length >= 2) {
      return [...trumpCards].sort((a, b) => {
        return cardCost(b, state.trump!) - cardCost(a, state.trump!)
      })[0]
    }
    const sideAce = cards.find((card) => {
      return card.rank === 'A' && effectiveSuit(card, state.trump!) !== state.trump
    })
  ...
  if (teamOf(currentWinner) === teamOf(player)) {
    return weakest(cards, state.trump)
  }

  const winningCards = cards.filter((card) => {
    return trickWinner([...state.trick, { card, player }], state.trump!) === player
  })
